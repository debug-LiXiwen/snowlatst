...
public class UserApi {
    ...
    public Map<String,Object> updateCstate(@RequestParam("state") Integer cstate,@RequestParam("userId") String userId){
        ...
        User user = new User();
        user.setCstate(cstate);
        user.setUserId(userId);

        data = userService.updateCstate(user);
        return data;

    }

    @RequestMapping(value = "/snow/clock/status/{userId}" , method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> selectSnowClock(@PathVariable("userId") String userId){
        logger.info("GET clock status : ");
        User user =new User();
        user.setUserId(userId);
        Map<String,Object> data = null;
        data = userService.selectSnowClock(user);
        return data;
    }

    @RequestMapping(value = "/snow/task/{userId}" , method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> selectTask(@PathVariable("userId") String userId){
        logger.info("GET snow task : ");
        User user =new User();
        user.setUserId(userId);
        Map<String,Object> data = null;
        data = userService.selectTask(user);
        return data;
    }

    @RequestMapping(value = "/snow/workTime/{userId}" , method = RequestMethod.GET)
    @ResponseBody
    public Map<String,Object> selectWorkTime(@PathVariable("userId") String userId){
        logger.info("GET snow workTime : ");
        User user =new User();
        user.setUserId(userId);
        Map<String,Object> data = null;
        data = userService.selectWorkTime(user);
        return data;
    }
